...
public class FileUploadService {
  ...
  public <T> CompletableFuture<List<T>> parseAndProcessFile(
          Processor<T,Path> processor, File file) {
    Path filePath = file.toPath();
    return processor.processData(filePath);
  }

  public <T> CompletableFuture<String> saveProcessedFileData(
          Processor<T,Path> processor, List<T> processedData) {
    return processor.saveProcessedData(processedData);
  }
